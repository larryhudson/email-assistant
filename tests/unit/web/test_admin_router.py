...
async def _seed_assistant(
    ...
    *,
    ...
    inbound: str = "sam@assistants.example.com",
...
async def test_assistants_list_shows_seeded_assistant(
    ...
    body = resp.text
    assert "a-1" in body
    assert "sam@assistants.example.com" in body
    ...
    *,
    ...
    session.add(
        EmailMessage(
            ...
            to_emails=["sam@assistants.example.com"],
            subject="dinner?",
    ...
    if status == "completed":
        ...
        session.add(
            EmailMessage(
                ...
                from_email="sam@assistants.example.com",
...
async def test_run_detail_html_shows_full_trace(
    ...
    async with sqlite_session_factory() as session:
        ...
        await _seed_thread_and_run(
            session,
            run_id="r-detail",
            inbound_body="Hi Sam, what's for dinner?",
...
async def test_run_detail_json_returns_structured_payload(
    ...
    assert payload["inbound"]["body_text"] == "Hi Sam, what's for dinner?"
