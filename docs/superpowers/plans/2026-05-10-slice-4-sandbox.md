...
- `docker-compose.yml` — bind-mount `./data` into the worker at `/path/to/email-assistant/data` so host bind-mount paths line up. (Verify whether this is already done from earlier slices; if so, no-op.)
